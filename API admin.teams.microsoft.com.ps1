<#
    ...
    .NOTES
    filename: ./API admin.teams.microsoft.com.ps1
    author: Wesley
    blog: techblik.nl
    created: 14-04-2022
...
if(!$refreshToken){
    ...
    try{
        ...
        $response = Invoke-RestMethod -Method POST -UseBasicParsing -Uri "https://login.microsoftonline.com/$tenantId/oauth2/devicecode" -ContentType "application/x-www-form-urlencoded" -Body "resource=$resource&client_id=$clientId"
        Write-Output $response.message
        $waited = 0
        while($true){
            try{
                $authResponse = Invoke-RestMethod -uri "https://login.microsoftonline.com/$tenantId/oauth2/token" -ContentType "application/x-www-form-urlencoded" -Method POST -Body "grant_type=device_code&resource=$resource&code=$($response.device_code)&client_id=$clientId" -ErrorAction Stop
...
$url = "https://teams.microsoft.com/api/mt/part/emea-03/beta/users/tenantWideAppsSettings"
$content = '{"appSettingsList":[],"isAppsPurchaseEnabled":false,"isLicenseBasedPinnedAppsEnabled":true}'
